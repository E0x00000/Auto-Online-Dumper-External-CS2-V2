...
void Offset::parseAndAssignConstants(const std::string& jsonString) {
...
DWORD Offset::findOffsetByName(const nlohmann::json& j, const std::string& category, const std::string& name)
{
    if (j.contains(category) && j[category].contains(name)) {
        ...
        if (value.is_number()) {
            ...
            return std::stoul(value.get<std::string>(), nullptr, 16);
        }
    }
...
std::string Offset::DownloadString2(const string& URL)
{
    ...
    if (interwebs) {
        ...
        if (urlFile) {
            ...
            // Fecha os handles do InternetOpen e InternetOpenUrl
            InternetCloseHandle(urlFile);
        }
        InternetCloseHandle(interwebs);
    }

    // Caso haja algum conteúdo, substituímos "|n" por "\r\n"
    return replaceAll2(rtn, "|n", "\r\n");
}
...
bool Offset::UpdateOffsets()
{
    try {
        ...
        parseAndAssignConstants(DownloadString2(url));
